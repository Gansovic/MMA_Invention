...
#db_name = '../resource/lib/publicdata/Code/db/video_database.db'
...
for video in video_list:
    print(video)
    filename, val = os.path.splitext(video)
    audio_file = filename = ".wav"
    fs, wav_data = wavfile.read(audio_file)
    ...
    if args.f == features[0]: 
        ...
        x = ft.extract_mfcc(frame_to_audio(0, get_frame_rate(video), fs, wav_data), fs)
    ...
    for i in range(len(frame)): 
        print('Match', i + 1,'at: ', frame[i] / frame_rate, 'seconds, with score of:', score[i]) 
        #print('Second best Match', i + 1,'at: ', frame[i] / frame_rate, 'seconds, with score of:', score[i])
        #print('Third best Match', i + 1,'at: ', frame[i] / frame_rate, 'seconds, with score of:', score[i])
